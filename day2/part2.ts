...
function getMin(setsOfCubes: { [key: string]: number }[] ): { [key: string]: number } {
  let colorsValues: { [key: string]: number } = {
    red: 0,
    green: 0,
    blue: 0,
  };
  setsOfCubes.map((cubes) => {
    for (let color in cubes) {
      if (cubes[color] > colorsValues[color]) {
        colorsValues[color] = cubes[color];
      }
    }
  });
  return colorsValues;
